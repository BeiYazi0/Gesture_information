...
NChirp=200;            % 每帧每Tx脉冲数
Doppler_Number=NChirp*Tx_Number*frame_num; % 总chirp数

%% 读取回波数据
fid = fopen(fname,'rb'); % 以二进制格式只读方式打开数据文件
bufferSize=Range_Number*Doppler_Number*Rx_Number*2; % 读取数据大小
...
for i=1:2:Doppler_Number
     ADC_Data(:,i,:)=ADC_Data(:,i,:)-B;
     B=B+a*ADC_Data(:,i,:);
end
B=0;
for i=2:2:Doppler_Number
     ADC_Data(:,i,:)=ADC_Data(:,i,:)-B;
     B=B+a*ADC_Data(:,i,:);
end
...
% imwrite(img,dstpath+'\range.png');
% oimg = getframe(2);
% img=imstand(oimg.cdata);            % 图像标准化
% imwrite(img,dstpath+'\doppler.png');
% oimg = getframe(3);
% img=imstand(oimg.cdata);            % 图像标准化
% imwrite(img,dstpath+'\angle.png');
