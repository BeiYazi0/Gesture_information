...
Tx_Number = 2;               %发射天线
Rx_Number = 4;               %接收天线
Range_Number = 256;          %距离点数（每个脉冲128个点）       512 
Doppler_Number = 16;        %多普勒通道数(总共128个重复脉冲数) 16  
global Params;
Params.NChirp = Doppler_Number;               %1帧数据的chirp个数
Params.NChan =  Rx_Number;                    %RxAn数,ADC通道数j
Params.NSample = Range_Number;                %每个chirp ADC采样数
Params.Fs = 5.209e6;                           %采样频率 5e6
Params.c = 3.0e8;                     %光速
Params.startFreq = 77e9;              %起始频率 
Params.freqSlope = 70e12;             %chirp的斜率 30e6
Params.bandwidth = 3.072e9;           %真实带宽
Params.lambda=Params.c/Params.startFreq;    %雷达信号波长
Params.Tc = 102.4e-6;                         %chirp周期 144e-6
global FFT2_mag;
...
if(loadCfg)
        ...
        configurationFileName_stop ='xwr16xx_profile_2022_07_13T07_10_23_642.cfg';             
...
figure();
t=(0:127)*5;
df = Params.Fs/Params.NSample;
f = (0:Params.NSample-1)*df;
[max_a,index]=max(FFT1_mag,[],2);
fif=f(index);
R=Params.c*fif/2/Params.freqSlope;
plot(t,R);
% mesh(FFT1_mag);
xlabel('时间/us');ylabel('距离/m');
...
figure();
df = 1/Params.Tc/Params.NChirp;
f=(-Params.NChirp/2:Params.NChirp/2-1)*df;
v=zeros(1,Params.NSample);
for k=index
    [max_b,p]=max(FFT2_mag(:,k));
    v(k)=f(p)*Params.lambda/2;
end
x=Params.c*(0:Params.NSample-1)*Params.Fs/2/Params.freqSlope/Params.NSample;
plot(x,v);
figure();
mesh(X,Y,FFT2_mag);
